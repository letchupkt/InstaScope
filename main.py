...
def printlogo():
    ...
    pc.printout("\nVersion 1.3 - Developed by @letchupkt\n\n", pc.YELLOW)
...
parser = argparse.ArgumentParser(description='InstaScope is an OSINT tool for Instagram. It offers an interactive shell '
                                             'to perform analysis on Instagram accounts by their nickname ')
# Remove positional argument for username
...
pc.printout("Enter the target username: ", pc.YELLOW)
target_username = input().strip()
while target_username == "":
    pc.printout("Target username cannot be blank. Please enter the target username: ", pc.YELLOW)
    target_username = input().strip()

api = InstaScope(target_username, args.file, args.json, args.command, args.output, args.cookies)
